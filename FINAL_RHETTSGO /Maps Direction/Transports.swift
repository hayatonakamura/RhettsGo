...
class Transports: UIViewController{
    ...
    func getDrivingData(origin: String, destination: String, completionHandler: @escaping ([String: Any]) -> ()){
        
        let jsonURLString = "https://maps.googleapis.com/maps/api/directions/json?"
        let key = "AIzaSyAYQYjp_C4gySyTcGvSlV2VgQu8gz6KTzE"
        
        
        let urlstring = jsonURLString + "origin=" + origin + "&destination=" + destination + "&key=" + key
        let urlrequest = URLRequest(url: URL(string: urlstring)!)
        let config = URLSessionConfiguration.default
        let sessions = URLSession(configuration: config)
