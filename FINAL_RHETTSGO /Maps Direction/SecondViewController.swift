...
class SecondViewController: UIViewController {
    ...
    func getwaittime(eta: String) -> Int{
        
        print(eta)
        let hours = Int(String(eta.prefix(2))) as! Int
        let minutes = Int(String(eta.suffix(2))) as! Int
        print(hours)
        print(minutes)
        var date = NSDate()
        var str = String(describing: date)
        ...
        var subHour = Int(String(myString.prefix(2))) as! Int
        subHour = subHour - 4
        if (subHour<0){
            subHour = subHour + 24
        }
        ...
        let subminutes = Int(String(myString.suffix(2))) as! Int
        print(subminutes)
        let differenceinmin = hours*12 + minutes - subHour*12 - subminutes as! Int
        print("Wait time is ")
    ...
    var originstoplatandlon = ""
    var destinationstoplatandlon = ""
    var waittime = 0
    ...
    override func viewDidLoad() {
       ...
       // Shuttlelabeleta.text = "Terrier Time!"
        ...
        var destinationstoplat  = 0 as Double
        var destinationstoplon = 0 as Double
        var originstoplat  = 0 as Double
        var originstoplon = 0 as Double
        print("1 for left")
        print(goingleft)
        if (goingleft == 1){
            for i in StopsGoingLeft{
                ...
                if (destinationmindist > self.getD(num1: [i.lon, i.lat], num2: [newlon2, newlat2])){
                    ...
                    destinationid = i.id
                    destinationstoplon = i.lon
                    destinationstoplat = i.lat
                }
        ...
        originstoplatandlon = String(Double(round(10000*originstoplat)/10000)) + ","
        originstoplatandlon = originstoplatandlon + String(Double(round(10000*originstoplon)/10000))
        
        destinationstoplatandlon = String(Double(round(10000*destinationstoplat)/10000)) + ","
        destinationstoplatandlon = destinationstoplatandlon + String(Double(round(10000*destinationstoplon)/10000))
        print("Here is " + originstoplatandlon + destinationstoplatandlon)
        ...
        shuttle.getShuttleData(){ json in
            ...
            if let ResultSet = json["ResultSet"] as? [String: Any]{
                if let Results = ResultSet["Result"] as? [[String: Any]]{
                    ...
                    self.waittime = self.getwaittime(eta: eta)
                    
                    cs.getWalkingData(origin: self.originstring, destination: self.originstoplatandlon){
                        json in
                        print("cando")
                        var results: [String] = []
                        var totaltime = ""
                                ...
                                if let legs = rout["legs"] as? [[String: Any]]{
                                    for leg in legs{
                                        if let timedetails = leg["duration"] as? [String: Any]{
                                            ...
                                            let durationwalk1 = Int(subtime) as Int!
                                            cs.getDrivingData(origin: self.self.originstoplatandlon, destination: self.self.destinationstoplatandlon){
                                                json in
                                                print("cando")
                                                var results: [String] = []
                                                var totaltime = ""
                                                        ...
                                                        if let legs = rout["legs"] as? [[String: Any]]{
                                                            for leg in legs{
                                                                if let timedetails = leg["duration"] as? [String: Any]{
                                                                    ...
                                                                    let durationdriving = Int(subtime) as Int!
                                                                    cs.getWalkingData(origin: self.destinationstoplatandlon, destination: self.destinationstring){
                                                                        json in
                                                                        print("cando")
                                                                        var results: [String] = []
                                                                        var totaltime = ""
                                                                                ...
                                                                                if let legs = rout["legs"] as? [[String: Any]]{
                                                                                    for leg in legs{
                                                                                        if let timedetails = leg["duration"] as? [String: Any]{
                                                                                            ...
                                                                                            let durationwalk2 = Int(subtime) as Int!
                                                                                            
                                                                                            let totalduration = durationwalk1! + durationwalk2! + durationdriving! + self.waittime
                                                                                            
                                                                                            print(totalduration)
                                                                                            totaltime = self.getETA(duration: totalduration)
                                                                                            print("HERE BITCH")
                                                                                            print("ETA for Shuttle is " + totaltime)
                                                                                            // self.walkinglabeleta.text = totaltime //set text for ETA label
                                                                                            DispatchQueue.main.async {
                                                                                                self.Shuttlelabeleta.text = totaltime
                                                                                            }
                                                                                        }
                                                                                    }
                                                                                }
